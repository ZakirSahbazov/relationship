...
async function enterMain() {
  ...
  if (inviterTgId) {
    // Davetle gelindi -> once backend'e sorulur, "zaten test edildi mi?"
    // Zaten edildiyse hic test ekranini gostermeden normal karsilamaya gec.
    const alreadyTested = await runInvitedMatch();
    if (alreadyTested) {
...
//    Geri donus degeri: true = "zaten test edilmisti, hicbir sey gosterilmedi"
//                        false = "test yapildi (veya hata oldu), reveal gosterildi"
// ---------------------------------------------------------------------------
async function runInvitedMatch() {
  ...
  try {
    ...
    if (result.already_tested) {
      // Bu ikili daha once test edilmis -> hicbir sey gosterme,
      // enterMain() normal karsilama ekranini acacak.
      return true;
    }
